...
uses
  ...
  SynHighlighterHTML, SynHighlighterCpp, SynPluginSyncroEdit, SynExportHTML,
  synhighlighterunixshellscript, Controls, Graphics, Dialogs, StdCtrls, Menus,
  ComCtrls, Buttons, Clipbrd, ValEdit, ExtCtrls, PairSplitter, ButtonPanel,
  EditBtn, DBCtrls, ActnList, ColorBox, Grids, Unit3, PQConnection, SQLite3Conn,
  Types, UITypes, IpHtml, ExtendedTabControls, CheckBoxThemed, TAGraph,
  TASources, TAStyles, PrintersDlgs, uPSComponent, ShellAPI, process,
  ServiceManager;
...
begin
     if (Editor.Highlighter = HTMSyn) or (Editor.Highlighter = FPCSyn) or
      (Editor.Highlighter = BATSyn) or
      (Editor.Highlighter = UNIXSyn) then
      Editor.Highlighter.Free;

    if (ExtractFileExt(editableFile) = '.html') or (ExtractFileExt(editableFile) = '.htm') then
      editor.Highlighter := HTMSyn;
        ...
        if (ExtractFileExt(editableFile) = '.pas') or
          ...
          (ExtractFileExt(editableFile) = '.inc') then

...
begin
  ...
  Form4.CheckBox3.Checked:=Editor.Visible;
...
begin
  ...
  Writeln(config, BoolToStr(Editor.Visible));
  WriteLn(config, '');
  WriteLn(config, '-------');
  WriteLn(config, '');
  ...
  Writeln(config, '21st string = Editor visibility');
  CloseFile(config);
end;
...
begin
  Editor.Lines.Add(GetEnvironmentVariable('USERNAME'));
  //USERNAME is for Windows. For *NIX systems, use the USER variable
end;
...
begin
  ...
  begin
    ...
    Editor.Visible:=StrToBool(configFile.Strings[21]);

...
uses
  ...
  Controls, Graphics, Dialogs, StdCtrls, Menus, ComCtrls, Buttons, Clipbrd,
  ValEdit, ExtCtrls, PairSplitter, ButtonPanel, EditBtn, DBCtrls, ActnList,
  Unit3, PQConnection, SQLite3Conn, Types, UITypes, IpHtml, ExtendedTabControls,
  CheckBoxThemed, TAGraph, TASources, PrintersDlgs, uPSComponent, ShellAPI,
...
begin
  ...
  begin
    ...
    begin
      ...
      MessageDlg ('Строка ' + fd.FindText + ' не найдена!', mtConfirmation, [mbOK], 0);
  end;
end;
...
begin
  ...
  begin // Нажата - производим замену во всем тексте
    ...
    if Editor.SelText <> '' then  // Если есть выделенный фрагмент - меняем его
...
procedure TForm1.EditorDropFiles(Sender: TObject; X, Y: integer;
  AFiles: TStrings);
begin
